def bit_truth(bin_arr, split_lfsr_17, split_lfsr_25):
	# The full-adder truth table below is utilized to determine the final bit values to be output.
	cin = 0
	cout = 0
	s = []
	crypt_bin = []
	...
	for i in range(len(bin_arr)):
		for j in range(len(bin_arr[i])):
			if split_lfsr_17[i][j] == 0 and split_lfsr_25[i][j] == 0 and cin == 0:
				...
				exit(0)
			cin = cout
			# Xor to obtain the bit to append.  Repeat until the loops finish, and return the final result.
